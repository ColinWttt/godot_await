...
use crate::framework::{bencher, BenchResult, RustBenchmark, RustTestCase, TestContext};
...
impl IntegrationTests {
    ...
    fn run_all_tests(
        ...
        #[cfg(since_api = "4.2")]
        {
            ...
            let on_finalize_test = move |stats| {
                let rust_async_time = clock.elapsed() - rust_time;
    ...
    fn run_async_rust_tests(
        ...
        scene_tree: Gd<Node>,
        // property_tests: Gd<Node>,
        on_finalize_test: impl FnOnce(TestStats) + 'static,
    ) {
        let mut tests_iter = tests.into_iter();

        let Some(first_test) = tests_iter.next() else {
            return on_finalize_test(stats);
        };
    ...
    fn conclude_tests(
        ...
        // }
        println!("  Time: Rust {rust_time:.2}s.");
